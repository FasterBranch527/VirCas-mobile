            ...
            Column(Modifier.fillMaxSize().padding(horizontal = 16.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (landscape) {
                    Row(Modifier.weight(1f).fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                        BlackjackFeltTable(round, dealerShown, holeRevealed, shownCounts, activeIndex, baseStake, committedStake, message, true, Modifier.weight(1.65f).fillMaxHeight())
                    ...
                    Box(Modifier.fillMaxWidth().heightIn(max = (maxHeight * .48f).coerceAtLeast(120.dp)).verticalScroll(rememberScrollState())) { controls() }
                }
            }
        }
    }
...
private fun BlackjackFeltTable(
    ...
    val dealerWidth = if (compact) 62.dp else 70.dp
    val playerWidth = if (hands.size > 1) 60.dp else if (compact) 72.dp else 80.dp
    Surface(modifier.testTag("blackjack-felt"), shape = RoundedCornerShape(28.dp), color = BlackjackInk, border = BorderStroke(1.dp, BlackjackGold.copy(alpha = .32f))) {
        BoxWithConstraints {
            ...
            Column(
                Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).padding(18.dp).heightIn(min = (maxHeight - 36.dp).coerceAtLeast(0.dp)),
                horizontalAlignment = Alignment.CenterHorizontally,
                verticalArrangement = Arrangement.SpaceBetween
            ) {
                BlackjackDealerArea(round?.dealer.orEmpty(), dealerShown, holeRevealed, dealerWidth, dealerWidth * 1.46f, compact)
                Box(Modifier.fillMaxWidth().padding(vertical = 16.dp)) { BlackjackRoundStatus(message, committedStake, hands.size.coerceAtLeast(1), compact) }
